...
namespace Game.Process
{
    ...
    public class PlayController : SingletonMonoBehaviour<PlayController>
    {
        ...
        // 在某个合适的地方调用PlayController的Init方法
        void Start()
        {
            PlayController.Instance.Init(4); // 假设轨道数量为4
        }
        ...
        private void TestMiss(int lane)
        {
            ...
            if(judgement.Out(gn))
            {
                ...
                //新增超时未点击的miss
                AutoPlaceTower.Instance.OnNoteJudged(-1,lane);
                //Debug.Log($"[TestMiss] 超时未点击，触发OnNoteJudged(-1)");
                ...
                if(gn.Type() == 2){
                    ...
                    if(cn != null){
                        if(cn == laneNotes[lane].Peek()){
                            ...
                            laneNotes[lane].Dequeue().Miss();
                            AutoPlaceTower.Instance.OnNoteJudged(-1,lane);
                            //Debug.Log($"[TestMiss] 长押第二个超时，触发OnNoteJudged(-1)");
                        }
                    }
                }
            }
        ...
        private void LaneKeyDown(int lane)
        {
            ...
            if (AutoPlaceTower.Instance != null)
            {
                AutoPlaceTower.Instance.OnNoteJudged(result, lane);
            }
            else
            {
                Debug.LogError("AutoPlaceTower.Instance is null.");
            }
        }
        ...
        private void LaneKeyUp(int lane)
        {
            if (laneHolding[lane])
            {
                ...
                //AutoPlaceTower.Instance.OnNoteJudged(result);
                AutoPlaceTower.Instance.OnNoteJudged(result, lane);
            }
